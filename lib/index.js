...
export default function () {
  ...
  instance.read_capabilities = (platform, tests_path) => {
   
    let capabilites_path = instance.capabilities_path(platform, tests);
    if(!instance.capabilities_exists) return err;

    let [err, caps] = jsonfile.readFileSync(caps_path);
    if(err) return [err];
  ...
  instance.lists_tests = (tests_path) => {
