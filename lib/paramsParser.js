...
export default function parser(argv = []) {
  ...
  const device_name = argv.indexOf("--device-name")
  if(device_name != -1) {
    settings["device_name"] = argv[device_name+1]; 
  } else {
    if (platform === 'android') {
      settings["device_name"] = "Android Emulator";
      return;
    }
    settings["device_name"] = "iOS Simulator";
  }
