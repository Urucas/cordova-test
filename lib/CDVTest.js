...
export default class CDVTest {
  ...
  constructor(settings) {
    ...
    if (!this.compile) this.app_path = glue([process.cwd(), settings.app_path]);
  }
  ...
  compileIos() {
    ...
    let appPath = glue([this.platform_path, 'build', 'emulator', appName]);
    let ipaPath = glue([this.platform_path, 'build', 'emulator', ipaName]);
    if (this.device) {
      appPath = glue([this.platform_path, 'build', 'device', appName]);
      ipaPath = glue([this.platform_path, 'build', 'device', ipaName]);
    }
    // compile ios
    this.logger.echo('\nCompiling cordova application, this may take a while!');
