...
export default class CDVTest {
  ...
  is_tests_path_reachable(tests_path) {
    try {
  ...
  checkSettings() {
    ...
    if(!this.is_tests_path_reachable(this.tests_path)) {
  ...
  compile_android() {
    let logger = this.logger
    let apk_path = glue([this.platform_path, "build/outputs/apk/","android-debug.apk"])
    if(!this.nocompile) {
      // compile android
  ...
  run_test(test, list) {
    ...
    let self = this 
    logger.echo("Running test; "+this.env+" "+test+" --platform "+this.platform)
    let child = child_process.spawn(this.env, params, { stdio: 'inherit' })
  ...
  run_tests(list) {
    ...
    let logger = this.logger
    if(!list.length) {
      logger.ok("All test finished!")
      process.exit(0)
      return
    }
    let test = list.shift()
    this.run_test(test, list)
  }
  ...
  test() {
    ...
    if(fs.lstatSync(this.tests_path).isFile()) {
      ...
      let [err, list] = finf(this.tests_path, this.env_files)
      if(err) { 
        logger.fail(err)
        process.exit(0)
      }
