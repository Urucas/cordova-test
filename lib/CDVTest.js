...
export default class CDVTest {
  ...
  constructor(settings) {
    ...
    if (this.nocompile) this.app_path = glue([process.cwd(), settings.app_path]);
  }
  ...
  getExistingCapabilities() {
    ...
    const capsPath = glue([testsPath, `${platform}.json`]);
    try {
  ...
  beforeCompile() {
    ...
    const info = {};
    try {
      const configPath = glue([process.cwd(), 'config.xml']);
  ...
  compile() {
    let err;
    let appPath;
    const err1 = this.beforeCompile();
    if (err1) return err1;

    if (this.platform === 'ios') {
      ([err, appPath] = this.compileIos());
    } else {
      ([err, appPath] = this.compileAndroid());
    }
    if (err) return err;

    const err2 = this.afterCompile(appPath);
    if (err2) return err2;
  ...
  afterCompile(appPath) {
    ...
    return new Error(`Compiled application not found; ${appPath}`);
  }

  compileAndroid() {
    const apkPath = glue([this.platform_path, 'build/outputs/apk/', 'android-debug.apk']);
    if (!this.nocompile) {
      // compile android
      ...
      this.logger.ok('Cordova project compiled');
    }
    return [null, apkPath];
  }
  ...
  compileIos() {
    ...
    const appPath = glue([this.platform_path, 'build', 'emulator', appName]);
    const ipaPath = glue([this.platform_path, 'build', 'emulator', ipaName]);
    if (!this.nocompile) {
      // compile ios
      this.logger.echo('\nCompiling cordova application, this may take a while!');
      let child = childProcess.spawnSync('cordova', ['build', 'ios']);
      if (child.stderr && child.stderr.toString() !== '') {
        return [new Error(child.stderr.toString())];
      }
      // build ipa
      ...
      child = childProcess.spawnSync('/usr/bin/xcrun', params);
      if (child.stderr && child.stderr.toString() !== '') {
        return [new Error(child.stderr.toString())];
      }
      this.logger.ok('Cordova project compiled');
  ...
  runTest(test, list) {
    const babelParams = ['--compilers', 'js:babel-core/register', '--require', 'babel-polyfill'];
    let params = [test, '--platform', this.platform];
    if (this.babel) params = params.concat(babelParams);
    const self = this;
  ...
  test() {
    ...
    if (fs.lstatSync(this.testsPath).isFile()) {
      ...
      for (let i = 0; i < list.length; i++) {
        const test = glue([this.testsPath, list[i]]);
        list[i] = test;
      }
      testsList = list;
    }
